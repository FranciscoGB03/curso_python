...
import com.example.instagramcloneandroid.api.CreateComment
import com.example.instagramcloneandroid.api.CreateCommentResponse
import com.example.instagramcloneandroid.api.CreatePost
import com.example.instagramcloneandroid.api.CreatePostResponse
import com.example.instagramcloneandroid.api.ImageUploadResponse
import com.example.instagramcloneandroid.api.InstagramApiService
import com.example.instagramcloneandroid.api.Post
import com.example.instagramcloneandroid.api.UserLoginResponse
import com.example.instagramcloneandroid.api.UserSignupRequest
import com.example.instagramcloneandroid.api.UserSignupResponse
...
class MainViewModel : ViewModel() {
                ...
                override fun onResponse(call: Call<List<Post>>, response: Response<List<Post>>) {
                    if (response.isSuccessful) {
                        val list = response.body()?.sortedByDescending { post ->
                            ...
                            val formartter = DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss")
                            val date = LocalDateTime.parse(split[0], formartter)
                            date
                        }
    ...
    /**
     * function to create a post. It includes an image and a comment
     *
     */
