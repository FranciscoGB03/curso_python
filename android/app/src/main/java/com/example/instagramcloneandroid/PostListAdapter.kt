...
import com.example.instagramcloneandroid.api.InstagramApiService.BASE_URL
import com.example.instagramcloneandroid.api.Post
import com.example.instagramcloneandroid.databinding.ItemPostBinding

class PostListAdapter(private var posts: List<Post>, val postCallback: PostCallback) :
    ...
    class PostViewHolder(val binding: ItemPostBinding, val postCallback: PostCallback) : RecyclerView.ViewHolder(binding.root) {
        fun bind(post: Post, loggedIn: Boolean) {
            ...
            val imageUrl =
                if (post.imageUrlType == "absolute") post.imageUrl else BASE_URL + post.imageUrl
